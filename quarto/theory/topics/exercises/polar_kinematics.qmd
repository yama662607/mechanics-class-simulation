...
```{ojs}
//| echo: false
viewof k     = Inputs.range([-0.5, 0.5], {step: 0.05, label: tex`\text{パラメータ } k`,  value: 0.15})
viewof omega = Inputs.range([0.5,  5.0], {step: 0.1,  label: tex`\text{角速度 } \omega`,       value: 1.5})
viewof t     = Inputs.range([0.0,  5.0], {step: 0.05, label: tex`\text{時間 } t \text{ (s)}`,     value: 0.0})
...
update_t = {
  ...
  while (animate) {
    let next_t = t + 0.05;
    if (next_t > 5.0) next_t = 0.0;
    viewof t.value = next_t;
    viewof t.dispatchEvent(new CustomEvent("input"));
...
theta_max = max(omega * t, 0.01)  # 0 除算・空配列を回避
...
# 現在時刻の質点の位置
x_current, y_current = x_array[-1], y_array[-1]

# 描画枠を t=5 s での最大半径に固定する（スライダー操作でサイズが変わらないように）
max_r = max(a, a * np.exp(abs(k) * omega * 5.0))
